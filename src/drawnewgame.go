...
var lastMove = `Last Move`
var winMove = `Win Move`
var depth = `- depth`
...
func drawAI(screen *ebiten.Image, g *game) {
	text.Draw(screen, artificial, mplusBigFont, newGameColumnBlack, row*8, color.Black)
	...
	text.Draw(screen, string(up), mplusBigFont, newGameColumnBlack+387, row*7, color.Black)
	text.Draw(screen, string(down), mplusBigFont, newGameColumnBlack+387, row*9, color.Black)

	text.Draw(screen, artificial, mplusBigFont, newGameColumnWhite, row*8, color.White)
	...
	text.Draw(screen, string(up), mplusBigFont, newGameColumnWhite+387, row*7, color.White)
	text.Draw(screen, string(down), mplusBigFont, newGameColumnWhite+387, row*9, color.White)
}

func drawHuman(screen *ebiten.Image, g *game) {
	text.Draw(screen, humanLower, mplusBigFont, newGameColumnBlack, row*4, color.Black)
	text.Draw(screen, humanLower, mplusBigFont, newGameColumnWhite, row*4, color.White)
}
...
func drawHighlight(screen *ebiten.Image, g *game) {
	...
	text.Draw(screen, highlight, mplusBigFont, newGameColumnWhite+800, row*2+22, color.Black)
	text.Draw(screen, lastMove, mplusBigFont, newGameColumnWhite+800, row*4, color.Black)
	text.Draw(screen, winMove, mplusBigFont, newGameColumnWhite+800, row*6, color.Black)
}
